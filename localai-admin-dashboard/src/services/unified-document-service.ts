...
export class UnifiedDocumentService {
  private static async getAuthenticatedProcessor(): Promise<DocumentProcessorEnhanced> {
    const dp = new DocumentProcessorEnhanced()
    const { access_token } = await requireAuthentication()
    dp.setAccessToken(access_token)
    return dp
  }
  ...
  private static async triggerAIAnalysis(documentId: string): Promise<void> {
    try {
      ...
      const hasValidTemplateForAnalysisSkip =
        document.metadata?.template_id && document.metadata.template_id !== 0
      if (hasValidTemplateForAnalysisSkip) {
        console.log(
          '🎯 Document has pre-selected template, skipping AI analysis and going directly to template extraction'
        )
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (attempt < maxAttempts - 1) {
          ...
          console.warn(
            `Storage download attempt ${attempt + 1}/${maxAttempts} failed, retrying in ${delayMs / 1000}s:`,
            result.error
          )
      ...
      if (downloadError || !fileData) {
        ...
        const hasValidTemplateForStorageFallback =
          document.metadata?.template_id && document.metadata.template_id !== 0
        if (hasValidTemplateForStorageFallback) {
          console.log(
            '🔄 Storage failed but template selected, attempting template extraction with fallback'
          )
          await this.updateDocumentStatus(documentId, {
            status: DocumentStatus.PROCESSING,
            metadata: {
          ...
          setTimeout(
            () => this.triggerTemplateExtractionWithFallback(documentId),
            100
          )
          return
        }
        ...
        await this.markDocumentFailed(
          documentId,
          'Failed to download file for analysis and no template selected'
        )
        return
      }
      ...
      // Initialize document processor with auth
      const documentProcessor = await this.getAuthenticatedProcessor()
      ...
      const hasValidTemplatePostAnalysis =
        document.metadata?.template_id && document.metadata.template_id !== 0
      if (hasValidTemplatePostAnalysis) {
        console.log(
          '🎯 Document has pre-selected template, triggering template extraction...'
        )
        setTimeout(() => this.triggerTemplateExtraction(documentId), 100)
      }
      // If we have template suggestions, automatically select and apply the best one (if confidence is high enough)
      else if (
        evaluation.template_suggestions &&
        evaluation.template_suggestions.length > 0
      ) {
        const bestTemplate = evaluation.template_suggestions.sort(
          (a, b) => b.match_score - a.match_score
        )[0]

        const MIN_AUTO_SELECT_CONFIDENCE = 0.7 // Require 70% match score for auto-selection
        ...
        if (bestTemplate.match_score >= MIN_AUTO_SELECT_CONFIDENCE) {
          ...
          setTimeout(
            () =>
              this.applyTemplateToDocument(
                documentId,
                bestTemplate.template_id,
                bestTemplate.template_name
              ),
            100
          )
        } else {
          ...
          console.log(
            '💡 User will need to manually select a template or create a new one'
          )
      ...
      // If no template suggestions, do generic text extraction
        ...
        console.log(
          '⚠️ No template suggestions available, performing generic text extraction...'
        )
        setTimeout(() => this.triggerGenericTextExtraction(documentId), 100)
      }
    } catch (error) {
      ...
      if (isNetworkError) {
        console.log(
          '🔄 Network error detected, attempting fallback analysis...'
        )
        try {
          ...
          if (document) {
            const fallbackEvaluation = {
              ...
              processing_recommendations: {
                ...
                suggested_action:
                  'Manual review recommended - backend unavailable',
                alternative_actions: [
                  'Browse templates',
            ...
            await this.updateDocumentStatus(documentId, {
              ...
              metadata: {
                ...
                ai_classification: {
                  primary_category:
                    fallbackEvaluation.type_evaluation.primary_type,
                  confidence_score:
                    fallbackEvaluation.type_evaluation.confidence,
                  detection_method:
                    fallbackEvaluation.type_evaluation.detection_method,
                },
                template_suggestions: fallbackEvaluation.template_suggestions,
                processing_recommendations:
                  fallbackEvaluation.processing_recommendations,
                analysis_completed_at: new Date().toISOString(),
            ...
            console.log(
              '✅ Fallback analysis completed for document:',
              documentId
            )
            return
          }
   ...
   */
  private static async triggerTemplateExtractionWithFallback(
    documentId: string
  ): Promise<void> {
    try {
      console.log(
        '🎯 Triggering fallback template extraction for document:',
        documentId
      )
      ...
      console.log(
        '⚠️ Storage unavailable, completing with template metadata only'
      )
      ...
      const fallbackResult = {
        ...
        extractedFields:
          template.smart_variables?.reduce(
            (acc, variable) => {
              acc[variable.name] = {
                value: '',
                confidence: 0,
      ...
      console.log(
        '✅ Fallback template extraction completed for document:',
        documentId
      )
    } catch (error) {
      console.error(
        '❌ Fallback template extraction failed for document:',
        documentId,
        error
      )
      await this.markDocumentFailed(
   ...
   */
  private static async triggerGenericTextExtraction(
    documentId: string
  ): Promise<void> {
    try {
      console.log(
        '📄 Triggering generic text extraction for document:',
        documentId
      )
      ...
      if (downloadError || !fileData) {
        console.error(
          'Failed to download file for generic extraction:',
          downloadError
        )
        await this.markDocumentFailed(
          documentId,
          'Failed to download file for generic text extraction'
        )
        return
      }
      ...
      // Initialize document processor with auth and perform basic text extraction
      const documentProcessor = await this.getAuthenticatedProcessor()
      ...
      const extractionResult =
        await documentProcessor.processDocumentWithDocling(file)

      console.log(
        '✅ Generic text extraction completed for document:',
        documentId
      )
      ...
      console.error(
        '❌ Generic text extraction failed for document:',
        documentId,
        error
      )
      await this.markDocumentFailed(
   ...
   */
  private static async triggerTemplateExtraction(
    documentId: string
  ): Promise<void> {
    try {
      ...
      if (templateId === undefined || templateId === null || templateId === 0) {
        console.log(
          '⚠️ No valid template ID found (got: ' +
            templateId +
      ...
      if (downloadError || !fileData) {
        console.error(
          'Failed to download file for template extraction:',
          downloadError
        )
        await this.markDocumentFailed(
          documentId,
      ...
      if (
        !template.smart_variables ||
        !Array.isArray(template.smart_variables) ||
        ...
        await this.markDocumentFailed(
          documentId,
          `Template ${template.name} (ID: ${template.id}) is missing smart_variables definition`
        )
        return
      }
      ...
      // Initialize document processor with auth and perform extraction
      const documentProcessor = await this.getAuthenticatedProcessor()
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () => reject(new Error('Template extraction timeout')),
          5 * 60 * 1000
        )
      })

      const extractionPromise = documentProcessor.processDocumentWithTemplate(
        file,
        template as any
      )
      ...
      await this.finalizeDocument(documentId, {
        ...
        extracted_fields: extractionResult?.extractedFields as Record<
          string,
          any
        >,
        ...
        metadata: {
          template_extraction_completed_at: new Date().toISOString(),
          template_used: template.name,
          field_count: Object.keys(extractionResult?.extractedFields || {})
            .length,
        },
      })
    } catch (error) {
      console.error(
        '❌ Template extraction failed for document:',
        documentId,
        error
      )
   ...
   */
  static async forceCompleteDocument(
    documentId: string
    ...
    try {
      ...
      if (status !== 'processing') {
        console.log(
          'Document is not in processing status, current status:',
          status
        )
        return document
      }
      ...
      if (processingStartedAt) {
        const elapsed = Date.now() - new Date(processingStartedAt).getTime()
        console.log(
          'Document has been processing for:',
      ...
      return await this.finalizeDocument(documentId, {
        content_text:
          document.content_text || 'Document processing was incomplete',
  ...
  static async autoCompleteIfProcessed(documentId: string): Promise<boolean> {
    try {
      ...
      const hasTemplateExtracted =
        !!document.metadata?.template_extraction_completed_at
      const hasProcessingCompleted =
        !!document.metadata?.processing_completed_at
      const hasProcessingStarted = !!document.metadata?.processing_started_at

      // Check processing time
      const processingTime =
        hasProcessingStarted && document.metadata?.processing_started_at
          ? Date.now() -
            new Date(document.metadata.processing_started_at).getTime()
          : 0

      // Check if document appears to be actually completed (must have meaningful content)
      const hasMeaningfulContent =
        hasContent && document.content_text && document.content_text.length > 50
      const isActuallyCompleted =
        hasMeaningfulContent ||
        hasExtractedFields ||
        hasTemplateExtracted ||
        hasProcessingCompleted

      if (isActuallyCompleted) {
        console.log(
          '🔄 Auto-completing document that appears to be processed but stuck:',
          documentId
        )

        await this.finalizeDocument(documentId, {
          content_text:
            document.content_text || 'Document processing completed',
          extracted_fields:
            document.extracted_fields ||
            document.metadata?.extracted_fields ||
            {},
          processing_method:
            document.metadata?.processing_method || 'auto_completed',
          metadata: {
            auto_completed: true,
            auto_completed_at: new Date().toISOString(),
            auto_completion_reason:
              'Document had processing results but was stuck in processing status',
            original_processing_method: document.metadata?.processing_method,
          },
        })
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          throw new Error(
            'Permission denied - please ensure you are signed in and have permission to upload documents'
          )
        }
      ...
      if (!data?.id) {
        console.error(
          'Document insert returned without ID:',
      ...
      try {
        ...
        console.warn(
          'File storage upload failed, continuing without storage:',
          uploadError
        )
  ...
  private static async uploadFileToStorage(
    ...
    try {
      ...
      if (
        ...
        error.message.includes('extended attributes')
      ) {
        console.warn(
          'Extended attributes not supported, trying alternative upload method...'
        )
        ...
        try {
          ...
          if (retryError) {
            throw new Error(
              `Failed to upload file (retry): ${retryError.message}`
            )
          }
        } catch (retryError) {
  ...
  static async updateDocumentStatus(
    ...
    try {
      ...
      if (fetchError) {
        throw new Error(
          `Failed to fetch current document: ${fetchError.message}`
        )
      }
      ...
      switch (options.status) {
        ...
        case DocumentStatus.PROCESSING:
          if (
            !updatedMetadata.analysis_completed_at &&
            updatedMetadata.analysis_started_at
          ) {
      ...
      const data = Array.isArray(rawUpdateData)
        ? rawUpdateData[0]
        : rawUpdateData
    ...
    try {
      return await UnifiedDocumentService.updateDocumentStatus(documentId, {
        ...
        metadata: {
          ...
          confidence_distribution:
            results.quality_metrics?.confidence_distribution,
        },
      })
    } catch (error) {
  ...
  static async getProcessingAnalytics(): Promise<{
    ...
    try {
      ...
      analytics.avgProcessingTime =
        completedDocs > 0 ? totalProcessingTime / completedDocs : 0
  ...
  static async selectBestTemplate(documentId: string): Promise<DocumentRecord> {
    try {
      ...
      const bestTemplate = suggestions.sort(
      ...
      // Apply the template
      return await this.applyTemplateToDocument(
        documentId,
        bestTemplate.template_id,
        bestTemplate.template_name
      )
