...
describe('Signup Confirmation Email Integration (Real API)', () => {
  ...
  describe('Welcome Email Integration', () => {
    ...
    it('should handle special characters in user names', async () => {
      ...
      const results = await Promise.all(
        specialNames.map(name => sendWelcomeEmail(`test.${Date.now()}@example.com`, name))
      );
  ...
  describe('Email Validation for Signup', () => {
    ...
    it('should sanitize email inputs properly', () => {
      const testCases = [
        ...
        { input: 'Test<Script>@Domain.Com', expected: 'testscript@domain.com' },
        { input: 'User>Alert@Example.Org', expected: 'useralert@example.org' },
        { input: 'NORMAL.EMAIL@DOMAIN.COM', expected: 'normal.email@domain.com' },
  ...
  describe('Signup Confirmation Flow Integration', () => {
    ...
    it('should handle concurrent welcome email sends', async () => {
      ...
      const promises = Array(concurrentCount).fill(0).map((_, i) => 
        sendWelcomeEmail(`concurrent.${i}.${Date.now()}@example.com`, `User ${i}`)
      );
  ...
  describe('Error Handling in Signup Flow', () => {
    ...
    it('should handle international characters in emails', async () => {
      ...
      const results = await Promise.all(
        internationalEmails.map(email => sendWelcomeEmail(email, 'Test User'))
      );
  ...
  describe('Performance and Rate Limiting', () => {
    ...
    it('should handle batch welcome emails efficiently', async () => {
      ...
      const promises = Array(batchSize).fill(0).map((_, i) => 
        sendWelcomeEmail(`batch.${i}.${Date.now()}@example.com`, `Batch User ${i}`)
      );
