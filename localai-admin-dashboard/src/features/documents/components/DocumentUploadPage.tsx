...
export function DocumentUploadPage({
  ...
  useEffect(() => {
    if (preSelectedTemplate) {
      console.log(
        '[DocumentUpload] Pre-selected template loaded:',
        preSelectedTemplate
      )
    }
  }, [preSelectedTemplate])
      ...
      try {
        ...
        if (result.action === 'generate_new' && result.generated_template) {
          ...
          if (
            ...
            if (!currentOrg) {
              console.warn(
                '[DocumentUpload] Cannot save template: No active organization (stale ref)'
              )
              toast.error(
                'Template generated but could not be saved — organization context was lost. Please try again.'
              )
            } else {
              try {
                const templateName =
                ...
                console.log(
                  '[DocumentUpload] Generated template saved:',
                ...
                console.error(
                  '[DocumentUpload] Failed to save generated template:',
                  saveErr
                )
                toast.error(
        ...
        console.log(
          '[DocumentUpload] Document finalized, navigating to:',
          documentId
        )
  ...
  const handleActionSelect = useCallback(
    async (action: ProcessingAction) => {
      ...
      try {
        ...
        console.log(
          '[DocumentUpload] Pre-selected template processing complete:',
          result
        )
        ...
        console.error(
          '[DocumentUpload] Pre-selected template processing failed:',
          err
        )
        setError(
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      console.log(
        '[DocumentUpload] handleFileSelect:',
        file.name,
        'preSelectedTemplate:',
        preSelectedTemplate
      )
      ...
      try {
        ...
        if (metadata?.storage_upload_failed) {
          console.warn(
            '[DocumentUpload] File storage failed, processing will continue:',
            metadata.storage_error
          )
        }
        ...
        if (preSelectedTemplate) {
          console.log(
            '[DocumentUpload] Using pre-selected template:',
            preSelectedTemplate
          )
          setIsProcessing(true)
          ...
          try {
            ...
            console.error(
              '[DocumentUpload] Pre-selected template error:',
              templateErr
            )
            setError(
              templateErr instanceof Error
                ? templateErr.message
                : 'Processing failed with selected template'
            )
            setIsProcessing(false)
            if (documentRecord.id) {
              setTimeout(
    ...
    <div className='container mx-auto space-y-6'>
        ...
        <div className='flex items-center gap-4'>
          <div>
            ...
            <p className='mt-1 text-gray-600 dark:text-gray-300'>
              Upload your document and let AI automatically detect its type and
              suggest the best processing approach.
            </p>
          </div>
        </div>
      </div>
        ...
        <Alert>
          ...
          <AlertDescription className='flex items-center justify-between'>
            <div>
              ...
              {preSelectedTemplate.name || `Template ${preSelectedTemplate.id}`}
              <span className='ml-2'>
      ...
      {!user && (
        <Alert>
          ...
          <AlertDescription>
            Please sign in to upload documents. Authentication is required for
            document processing.
          </AlertDescription>
        </Alert>
      )}
      ...
      {user && !isLoadingOrg && !activeOrganization && (
        <Alert>
          <AlertTriangle className='h-4 w-4' />
          <AlertDescription>
            No organization selected. Please select or create an organization to
            upload documents.
          </AlertDescription>
        </Alert>
      )}
            ...
            <div className='flex items-center justify-center space-x-3'>
              ...
              <div className='text-center'>
                <p className='text-lg font-medium text-gray-900 dark:text-white'>
                  Processing with {preSelectedTemplate.name}
                </p>
            ...
            <div className='text-center'>
              {preSelectedTemplate ? (
                <>
                  ...
                  <div className='mx-auto max-w-2xl space-y-1 text-sm text-gray-600 dark:text-gray-300'>
                    <p>
                      - <strong>Template Selected:</strong>{' '}
                      {preSelectedTemplate.name ||
                        `Template ${preSelectedTemplate.id}`}
                    </p>
                    <p>
                      - <strong>Direct Processing:</strong> Your document will
                      be processed immediately using this template
                    </p>
                    <p>
                      - <strong>Fast Results:</strong> No evaluation needed -
                      we'll extract data based on the template fields
                    </p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div className='mx-auto max-w-2xl space-y-1 text-sm text-gray-600 dark:text-gray-300'>
                    <p>
                      - <strong>Automatic Detection:</strong> AI identifies
                      document type (invoices, contracts, reports, etc.)
                    </p>
                    <p>
                      - <strong>Template Matching:</strong> Suggests the best
                      templates based on document content
                    </p>
                    <p>
                      - <strong>Intelligent Processing:</strong> Chooses optimal
                      extraction method for your document
                    </p>
                  </div>
                </>
              )}
