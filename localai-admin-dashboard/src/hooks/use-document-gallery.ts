...
function transformDocument(doc: any): Document {
  ...
  return {
    ...
    document_type:
      ...
      doc.metadata?.classification?.primary_category,
...
async function fetchDocuments(
  ...
  return withAuthentication(async (user) => {
    ...
    if (filters.search) {
      query = query.or(
        `name.ilike.%${filters.search}%,content_text.ilike.%${filters.search}%`
      )
    }
    ...
    if (filters.status && filters.status.length > 0) {
      ...
      const processingStatuses = filters.status.flatMap(
        (s) => statusMap[s] || []
      )
    ...
    if (filters.document_type && filters.document_type.length > 0) {
      ...
      const orConditions = filters.document_type
        .map(
          (type) =>
...
async function fetchDocumentStats() {
  return withAuthentication(async (user) => {
    ...
    data.forEach((doc) => {
      ...
      // Count by file type
      stats.by_file_type[doc.file_type] =
        (stats.by_file_type[doc.file_type] || 0) + 1
      ...
      const docType =
        ...
        doc.metadata?.classification?.primary_category
      if (docType) {
        stats.by_document_type[docType] =
          (stats.by_document_type[docType] || 0) + 1
      }
    })
      ...
      subscription = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (payload.eventType === 'DELETE') {
              console.log(
                'DELETE event detected via realtime, skipping cache invalidation to prevent conflicts'
              )
              return
            }
            ...
            console.log(
              'Invalidating queries due to realtime update:',
              payload.eventType
            )
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIPTION_ERROR') {
            console.warn(
              'Real-time subscription failed. Document gallery will still work but without live updates.'
            )
  ...
  const deleteMutation = useMutation({
    mutationFn: async (documentId: number) => {
      return withAuthentication(async (user) => {
        ...
        console.log(
          '🔍 Making authenticated Supabase query to documents table...'
        )
        ...
        console.log(
          '🗄️ Making DELETE request to documents table for ID:',
          documentId
        )
        ...
        if (dbError) {
          ...
          console.error(
            'CRITICAL: Database deletion failed after file deletion:',
            dbError
          )
          ...
          throw new Error(
            `File deleted but database cleanup failed. Please contact support. Error: ${dbError.message}`
          )
        }
    ...
    onSuccess: (result) => {
      ...
      const sizeInfo = result.freedSpace
        ? ` (${formatFileSize(result.freedSpace)} freed)`
        : ''
      toast.success(
        `Document "${result.documentName}" deleted successfully${sizeInfo}`
      )
    },
    onError: (error) => {
      ...
      if (error instanceof Error) {
        if (error.message.includes('not found')) {
          ...
          errorMessage =
            'Document file deleted but database cleanup failed. Please contact support.'
        } else {
