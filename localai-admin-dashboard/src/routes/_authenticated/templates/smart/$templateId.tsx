...
export const Route = createFileRoute('/_authenticated/templates/smart/$templateId')({
...
function SmartTemplateViewPage() {
  ...
  console.log('SmartTemplateViewPage: Component rendered', { templateId, mode });
  ...
  const { 
    ...
    queryFn: async () => {
      console.log('🔄 Fetching fresh template data for ID:', templateId);
      const foundTemplate = await smartTemplateService.getTemplate(Number(templateId));
      if (!foundTemplate) {
        throw new Error('Smart template not found');
      }
      console.log('✅ Fresh template data loaded:', foundTemplate.name, 'Variables:', foundTemplate.smart_variables?.length || 0);
  ...
  useEffect(() => {
    if (mode !== 'edit' && window.location.search.includes('mode=view')) {
      console.log('🧹 Cleaning up leftover query parameters');
      navigate({ 
  ...
  const hasActiveChildRoute = matches.some(match => 
    match.routeId.includes('/edit') || 
    match.routeId !== '/_authenticated/templates/smart/$templateId'
  );

  // If we have an active child route, just render the outlet
  if (hasActiveChildRoute && matches[matches.length - 1]?.routeId !== '/_authenticated/templates/smart/$templateId') {
    return <Outlet />;
  }
