...
export const Route = createFileRoute('/_authenticated/templates/smart/$templateId/edit')({
...
function SmartTemplateEditPage() {
  ...
  const { 
    ...
    queryFn: async () => {
      return await smartTemplateService.getTemplate(Number(templateId));
    },
    enabled: !!templateId,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async (templateData: SmartTemplate) => {
      if (template?.id) {
        return await smartTemplateService.updateTemplate(template.id, templateData);
      } else {
