...
test.describe('Reproduction: Template Saving Error', () => {
    test('should show error when auto-generated template fails to save due to missing org context', async ({ page }) => {
        ...
        await page.route('**/processing-stream', async (route) => {
            console.log('Mocking SSE stream response');
            await route.fulfill({
                status: 200,
        ...
        // We expect to see the error toast.
        const errorToast = page.getByText('Template was generated but failed to save');

        // START OBSERVATION
        try {
            await expect(errorToast).toBeVisible({ timeout: 10000 });
            console.log('✅ REPRODUCTION SUCCESS: Error toast appeared as expected.');
        } catch (e) {
            console.log('❌ REPRODUCTION FAILED: Error toast did not appear.');
            // Check if success message appeared?
            // "Document finalized" log in console or success toast
            if (await page.getByText('Document processed successfully').isVisible()) {
                console.log('Document processed successfully (Unexpected for reproduction)');
            }
        }

        // 6. Screenshot for proof
        await page.screenshot({ path: 'repro-failure.png' });

        // Assert availability to fail test if not reproduced
        // await expect(errorToast).toBeVisible(); 
        // We conditionally fail or just log for now to confirm "reproduction".
    });
});
