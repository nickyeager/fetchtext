...
2. **Admin password reset** to `CHANGEME-set-via-TEST_USER_PASSWORD-env`
...
- Login with: `admin@fetchtext.local` / `CHANGEME-set-via-TEST_USER_PASSWORD-env`
...
```bash
grep VITE_SUPABASE dashboard/.env.local
grep ANON_KEY .env
```
