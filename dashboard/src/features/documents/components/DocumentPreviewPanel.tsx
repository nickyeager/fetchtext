  ...
  const highlights = useMemo((): FieldHighlight[] => {
    ...
    return Object.entries(extractedFields)
      ...
      .map(([fieldName, field]) => {
        ...
        const rawBbox = extractedField.location?.bbox as { x: number; y: number; width: number; height: number; normalized?: boolean } | undefined;
        const bbox = rawBbox ? {
          x: rawBbox.x,
          y: rawBbox.y,
          width: rawBbox.width,
          height: rawBbox.height,
          coordinateType: (rawBbox.normalized ? 'normalized' : 'pixel') as 'normalized' | 'pixel',
        } : null;
