...
export function useDocumentDetail({
  ...
  const [fieldPositions, setFieldPositions] = React.useState<
    ...
    >
  >(new Map())
  // Page dimensions from backend (PDF points) for coordinate scaling
  const [pdfPageDimensions, setPdfPageDimensions] = React.useState<
  ...
  useEffect(() => {
    const fetchFieldPositions = async () => {
      ...
      try {
        ...
        setPdfPageDimensions(result.page_dimensions || {})
      } catch (error) {
        // eslint-disable-next-line no-console
        console.error(
